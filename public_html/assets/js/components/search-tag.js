...
class TagSearch {
  ...
  onInput() {
    ...
    // Получаем все .graff-group
    const groups = document.querySelectorAll(".graff-group");

    groups.forEach((group) => {
      const conts = group.querySelectorAll(".graff-cont");
      let anyMatch = false;

      conts.forEach((cont) => {
        const tags = cont.querySelectorAll(".graff-tag__text");
        const matchFound = Array.from(tags).some((tag) =>
          tag.textContent.trim().toLowerCase().includes(inputValue)
        );

        cont.style.display = inputValue
          ? matchFound
            ? "block"
            : "none"
          : "block";
      ...
      // Управление кнопкой: если хотя бы один .graff-cont видим — скрываем кнопку
      const loadMoreButton = group.querySelector(".graff-load-more");
      if (loadMoreButton) {
        const anyVisibleCont = Array.from(conts).some(
          (cont) => cont.style.display !== "none"
        );
        loadMoreButton.style.display = anyVisibleCont ? "none" : "inline-block";
      }

      // Показываем или скрываем всю группу, если нужно
      group.style.display = !inputValue || anyMatch ? "block" : "none";
    });
  ...
  getTagContainers() {
    ...
    tagElements.forEach((tagEl) => {
      ...
      const group = tagEl.closest(".graff-group");
