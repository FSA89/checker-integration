...
use App\Repository\api\RegionRepository;
...
{
    #[Route('', methods: ['GET'])]
    #[OA\Get(summary: "Получить регион по ID", tags: ["Region"])]
    ...
    #[OA\Response(response: 200, description: "Регион найден", content: new OA\JsonContent(
        type: 'array',
        items: new OA\Items(
            properties: [new OA\Property(property: 'domain_id', type: 'integer'),
                new OA\Property(property: 'region_name', type: 'string'),
                new OA\Property(property: 'region_id', type: 'number'),]
        )))]
    #[OA\Response(response: 404, description: "Регион не найден")]
    final public function index(RegionRepository $regionRepository, Request $request): JsonResponse
    {
        $query = $request->get('query');
        $data = json_decode($query, true);
        if (is_null($data)) {return $this->json(['status' => 'error','message'=>'uncorrected json'],400);}
        $answer = $regionRepository->findByDomainId((int)$data['id']);
